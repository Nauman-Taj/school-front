"use client";

import { useMemo, useState } from "react";
import {
  Bell,
  CheckCircle,
  Clock,
  FileText,
} from "lucide-react";

import { parentNotifications } from "@/data/parentNotifications";
import { getCurrentParentChildren } from "@/lib/parent";
import ParentNotificationTable from "./ParentNotificationTable";

export default function ParentNotificationsPage() {
  const children = getCurrentParentChildren();

  const [selectedChild, setSelectedChild] = useState(
    children[0]?.id ?? 0
  );

  const notifications = useMemo(
    () =>
      parentNotifications.filter(
        (notification) =>
          notification.childId === selectedChild
      ),
    [selectedChild]
  );

  const unread = notifications.filter(
    (notification) => !notification.read
  ).length;

  const read = notifications.filter(
    (notification) => notification.read
  ).length;

  const stats = [
    {
      title: "Total",
      value: notifications.length,
      icon: Bell,
      iconClass: "bg-[#e6f4f2] text-[#01796f]",
    },
    {
      title: "Unread",
      value: unread,
      icon: Clock,
      iconClass: "bg-yellow-50 text-yellow-600",
    },
    {
      title: "Read",
      value: read,
      icon: CheckCircle,
      iconClass: "bg-green-50 text-green-600",
    },
    {
      title: "Updates",
      value: notifications.filter(
        (notification) =>
          notification.type === "Announcement"
      ).length,
      icon: FileText,
      iconClass: "bg-blue-50 text-blue-600",
    },
  ];

  return (
    <div className="space-y-5">
      {/* Heading */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">
          Notifications
        </h1>

        <p className="mt-1 text-sm text-gray-500">
          View important notifications and updates for your child.
        </p>
      </div>

      {/* Child Selector */}
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        <label
          htmlFor="child"
          className="mb-2 block text-sm font-medium text-gray-700"
        >
          Select Child
        </label>

        <select
          id="child"
          value={selectedChild}
          onChange={(e) =>
            setSelectedChild(Number(e.target.value))
          }
          className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-700 outline-none transition focus:border-[#01796f] sm:w-80"
        >
          {children.map((child) => (
            <option key={child.id} value={child.id}>
              {child.name} --- {child.className}
            </option>
          ))}
        </select>
      </div>

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => {
          const Icon = stat.icon;

          return (
            <div
              key={stat.title}
              className="rounded-2xl border border-gray-200 bg-white p-5"
            >
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-gray-500">
                  {stat.title}
                </p>

                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-xl ${stat.iconClass}`}
                >
                  <Icon size={19} />
                </div>
              </div>

              <h2 className="mt-3 text-2xl font-bold text-gray-900">
                {stat.value}
              </h2>
            </div>
          );
        })}
      </div>

      {/* Notifications */}
      <div>
        {/* <h2 className="mb-4 text-lg font-semibold text-gray-900">
          Notifications
        </h2> */}

        <ParentNotificationTable
          notifications={notifications}
        />
      </div>
    </div>
  );
}